...
class ProfileEngine {
  ...
  _generate() {
    ...
    const chromeVer = r.nextInt(130, 137);
    ...
    const { lang: language, langs: languages } = this._genLanguage(r);
  ...
  _genUA(os, chromeVer, r) {
    ...
    const winVer = '10.0'; // Chrome always reports NT 10.0 even on Win11
    ...
    // Build full-version brands once for reuse
    const fullBrands = [
    ...
    // Flat high-entropy values (needed for DNR header rules in background.js)
    var _platVer = os === 'windows' ? `${winVer}.0` : (os === 'macos' ? '14.7.0' : '6.8.0');

    const userAgentData = {
      brands,
      mobile: false,
      platform: platformNames[os],
      platformVersion: _platVer,
      architecture: 'x86',
      bitness: '64',
 model: '',
